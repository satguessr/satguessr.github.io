...
Promise.all([
  $.getJSON('https://raw.githubusercontent.com/lmfmaier/cities-json/refs/heads/master/cities500.json'),
  $.getJSON('https://raw.githubusercontent.com/satguessr/satguessr.github.io/refs/heads/main/ne_10m_land.json')
]).then(([data1, data2]) => {
  // Assign the data to variables
  cityData = data1;
  landData = data2;

  // Enable the start button or perform other actions
  $('#startBtn').prop('disabled', false).text('Start Game');
  console.log('Both data sets loaded:', cityData, landData);
  console.log(tryGen("hi", 0));
});
...
const GuessMap = L.map('guessMapCanvas', { center: [20, 0], zoom: 0 })
  .addLayer(L.tileLayer('https://{s}.basemaps.cartocdn.com/light_nolabels/{z}/{x}/{y}{r}.png', baseLayerOpts));
const ResultMap = L.map('resultMapCanvas', { center: [20, 0], zoom: 0 })
  .addLayer(L.tileLayer('https://{s}.basemaps.cartocdn.com/light_nolabels/{z}/{x}/{y}{r}.png', baseLayerOpts));
const FinalMap = L.map('finalMapCanvas', { center: [20, 0], zoom: 0 })
  .addLayer(L.tileLayer('https://{s}.basemaps.cartocdn.com/light_nolabels/{z}/{x}/{y}{r}.png', baseLayerOpts));
...
function tryGen(seed, recurseIndex) {
  ...
  if(isOnLand){
    ...
    console.log("wrong");
    return tryGen(seed, recurseIndex + 1);
  }
}
